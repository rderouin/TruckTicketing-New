...
{
    internal List<IFilterCompoent> ActiveFilters { get; set; } = new();
}
...
public partial class GridFilterPanel<TItem> : GridFilterPanel
{
    ...
    private readonly Dictionary<string, Type> _filterOptionTypes = new()
    {
        ...
        { nameof(NumericFilter), typeof(NumericFilterOptions) },
        { nameof(DoubleFilter), typeof(DoubleFilterOptions) },
        ...
        { nameof(DropDownFilter), typeof(DropDownFilterOptions) },
        { nameof(SingleSelectDropDownFilter), typeof(SingleSelectDropDownFilterOptions) },
    };
    ...
    private readonly Dictionary<string, Type> _filterTypes = new()
    {
        ...
        { nameof(NumericFilter), typeof(NumericFilter<>) },
        { nameof(DoubleFilter), typeof(DoubleFilter<>) },
        ...
        { nameof(DropDownFilter), typeof(DropDownFilter<>) },
        { nameof(SingleSelectDropDownFilter), typeof(SingleSelectDropDownFilter<>) },
    };
    ...
    private void InitEventHandlerFactory()
    {
        _filterChangeEventFactory.Add(typeof(CheckBoxListFilter<>),
                                      () => _eventCallbackFactory
                                         .Create<FilterEventArgs<CheckBoxListFilter>>(this, OnfilterChanged));

        _filterChangeEventFactory.Add(typeof(DateFilter<>),
                                      () =>
                                          _eventCallbackFactory
                                             .Create<FilterEventArgs<DateFilter>>(this, OnfilterChanged));

        _filterChangeEventFactory.Add(typeof(NumericFilter<>),
                                      () =>
                                          _eventCallbackFactory
                                             .Create<FilterEventArgs<NumericFilter>>(this, OnfilterChanged));

        _filterChangeEventFactory.Add(typeof(DoubleFilter<>),
                                      () =>
                                          _eventCallbackFactory
                                             .Create<FilterEventArgs<DoubleFilter>>(this, OnfilterChanged));

        _filterChangeEventFactory.Add(typeof(RadioListFilter<>),
                                      () =>
                                          _eventCallbackFactory
                                             .Create<FilterEventArgs<RadioListFilter>>(this, OnfilterChanged));

        _filterChangeEventFactory.Add(typeof(TextboxFilter<>),
                                      () =>
                                          _eventCallbackFactory
                                             .Create<FilterEventArgs<TextboxFilter>>(this, OnfilterChanged));

        _filterChangeEventFactory.Add(typeof(TypeAheadFilter<>),
                                      () =>
                                          _eventCallbackFactory
                                             .Create<FilterEventArgs<TypeAheadFilter>>(this, OnfilterChanged));

        _filterChangeEventFactory.Add(typeof(DropDownFilter<>),
                                      () =>
                                          _eventCallbackFactory
                                             .Create<FilterEventArgs<DropDownFilter>>(this, OnfilterChanged));

        _filterChangeEventFactory.Add(typeof(SingleSelectDropDownFilter<>),
                                      () => _eventCallbackFactory
                                         .Create<FilterEventArgs<SingleSelectDropDownFilter>>(this, OnfilterChanged));
    }
    ...
    private void CreateFilterRenderFragments(List<IGrouping<int, GridFilter>> filterData)
    {
        ...
        {
            _filterRenderFragments.Add(new());
            ...
            foreach (var filter in group)
            {
                ...
                var filterRendering = ComponentGenerator.CreateDynamicComponent(filterType, new()
                {
                        ...
                        .Select(x =>
                                {
                                    ...
                                    if (x.FilterOptions is JObject jObj)
                                        filterOptions =
                                            (FilterOptions)
                                            ((JObject)x.FilterOptions).ToObject(_filterOptionTypes[x.FilterType]);
                                    else if (x.FilterOptions is FilterOptions fo) filterOptions = fo;
                                    ...
                                    return new GridFilter
                                    {
                                        ...
                                        FilterType = _filterTypes.ContainsKey(x.FilterType)
                                                         ? _filterTypes[x.FilterType]
                                                         : null,
    ...
    {
        foreach (var activeFilter in ActiveFilters) await activeFilter.Reset(SearchCriteria);
