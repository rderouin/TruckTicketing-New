...
{
    public abstract class DropDownFilter : ListFilterComponentBase<DropDownFilterOptions, FilterEventArgs<DropDownFilter>> { }

    public partial class DropDownFilter<TOutputValue> : DropDownFilter
    {
        ...
        public override Task<bool> IsValid()
        {
            try
            {
                if (Value == null || !Value.Any()) return Task.FromResult(true);
        ...
        public IEnumerable<string> bindValue = new List<string>();

        //public List<ListOption<string>> bindValue = new();
        ...
        {
            var oldValue = Value;
            Value = (IEnumerable<TOutputValue>)newValue;
        ...
        private async Task RaisOnChangeEvent(object oldValue, object newValue)
        {
            ...
            {
                await OnChange.InvokeAsync(new FilterEventArgs<DropDownFilter>(this)
                {
