            ...
            if (objFiltersKeyValues != null)
            {
                foreach (var kp in model?.Filters)
                {
                    if (kp.Value != null)
                    {
                        ...
                        {
                            if (obj.ContainsKey(nameof(AxiomModel.Field)) || obj.ContainsKey(nameof(AxiomModel.Field).ToLower()))
                            {
                                model.Filters[kp.Key] = JsonConvert.DeserializeObject<AxiomModel>(obj.ToString(), _jsonSettings);
                            }
                            else if (obj.ContainsKey(nameof(AxiomFilterModel.Axioms)) || obj.ContainsKey(nameof(AxiomFilterModel.Axioms).ToLower()))
                            {
