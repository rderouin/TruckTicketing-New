...
namespace Trident.Data
{
    ...
    ///     Implements the <see cref="TridentOptionsBuilder.Data.Contracts.IAbstractContextFactory" />
    ///     Implements the <see cref="System.IDisposable" />
    /// </summary>
    /// <seealso cref="TridentOptionsBuilder.Data.Contracts.IAbstractContextFactory" />
    /// <seealso cref="System.IDisposable" />
    /// <remarks>
    ///     This class maintains a cache of context objects. Generally, this class is
    ...
    public class AbstractContextFactory : IAbstractContextFactory, IDisposable
    {
        ...
        ///     The context cache
        /// </summary>
        private readonly ConcurrentDictionary<int, object> _contextCache = new ConcurrentDictionary<int, object>();
        ...
        /// <returns>T.</returns>
        public T Create<T>(Type entityType, bool useCache = true)
        {
            var context = default(T);
            var attr = entityType.GetCustomAttribute<UseSharedDataSourceAttribute>();

            var dataSourceName = !string.IsNullOrWhiteSpace(attr?.DataSource)
                                     ? attr.DataSource
                                     : SharedDataSource.DefaultDB.ToString();

            var contextHash = dataSourceName.GetHashCode();

            object ContextValueFactory(int hash)
            {
                var factory = (ISharedContextFactory<T>)_serviceLocator.GetNamed<ISharedContextFactory>(dataSourceName);
                return factory.Get(entityType, dataSourceName);
            }

            context = (T)(useCache
                              ? _contextCache.GetOrAdd(contextHash, ContextValueFactory)
                              : ContextValueFactory(contextHash));
