    ...
    internal static class AxiomFilterExtensions
    {
        ...
        public static Expression<Func<T, bool>> ToExpression<T>(this AxiomFilter filter)
        {
            ...
            using (var reader = new StringReader(filter.Format))
            {
                ...
                object expObj = processStack.Pop();

                var exp = expObj as Expression<Func<T, bool>> ?? GetAxiomFunc(expObj);               
        ...
        private static LambdaExpression CreateExpression(Type type, string propertyName, ParameterExpression paramExpression = null)
        {
            var param = paramExpression ?? Expression.Parameter(type, "x");
        ...
        {
            var type = typeof(T);
            paramExpression = paramExpression ?? Expression.Parameter(type, "x");
            var a = axiom;
            ...
            var keyPropertyExpression = CreateExpression(type, fieldName, paramExpression);
            var compareExpressionFunc = _operatorDict[a.Operator];

            var filterValue = ResolveTypeBoxing(keyPropertyExpression.Body.Type, a.Value);
            var constantExpression = keyPropertyExpression.Body.Type.IsNullableMember() 
                                         ? Expression.Constant(filterValue, keyPropertyExpression.Body.Type) 
                                         : Expression.Constant(filterValue);

            var equalExpression = compareExpressionFunc(keyPropertyExpression.Body, constantExpression);
        ...
        private static object ResolveTypeBoxing(Type targetType, object value)
        {
            ...
            else if (filterValueType == typeof(string))
            {
