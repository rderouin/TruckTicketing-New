...
namespace Trident.Business
{
        ...
        where TCriteria : SearchCriteria
    {
                                                    ...
                                                    bool applyDefaultFilters = true)
        {
            ...
            var obj = (await base.Get(filters, null, DefaultIncludedProperties, detach, loadChildren)).FirstOrDefault();
        ...
        public override async Task<TEntity> GetById(CompositeKey<object> key,
                                                    bool detach = false,
                                                    bool loadChildren = false,
                                                    bool applyDefaultFilters = true)
        {
            ...
            if (!(DefaultIncludedProperties?.Any() ?? false))
            {
                return await base.GetById(key, detach, loadChildren, applyDefaultFilters);
            }
            ...
            var obj = (await base.Get(filters, key.PartitionKey, null, DefaultIncludedProperties, detach, loadChildren)).FirstOrDefault();
        ...
        public virtual async Task<IEnumerable<TEntity>> GetByIds(IEnumerable<TId> ids, bool detach = false, bool loadChildren = false, bool applyDefaultFilters = true)
        {
            ...
            var entities = (await base.Get(filters, null, DefaultIncludedProperties, detach, loadChildren)).ToList();
        ...
        public virtual async Task<IEnumerable<TEntity>> GetByIds(IEnumerable<CompositeKey<TId>> keys, bool detach = false, bool loadChildren = false, bool applyDefaultFilters = true)
        {
            var allEntities = new List<TEntity>();

            foreach (var batch in keys.GroupBy(k => k.PartitionKey))
            {
                var batchIds = batch.Select(k => (object)k.Id).ToList();
                var batchPartitionKey = batch.Key;

                Expression<Func<TEntity, bool>> expression = x => batchIds.Contains(x.Id);
                var filters = (applyDefaultFilters)
                                  ? ApplyDefaultFilters(expression)
                                  : expression;

                var entities = (await base.Get(filters, batchPartitionKey, null, DefaultIncludedProperties, detach, loadChildren)).ToList();
