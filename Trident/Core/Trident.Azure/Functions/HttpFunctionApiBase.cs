...
{
    public abstract class HttpFunctionApiBase<TModel, TEntity, TId> : ReadOnlyHttpFunctionApiBase<TModel, TEntity, TId>,
                                                                      IFunctionController
        where TEntity : EntityBase<TId>
        where TModel : ApiModelBase<TId>
    {
        protected HttpFunctionApiBase(ILog log, IMapperRegistry mapper, IManager<TId, TEntity> manager) :
            base(log, mapper, manager)
        {
        }

        protected async Task<HttpResponseData> Update(HttpRequestData req,
                                                      TId id,
                                                      string logName,
                                                      Func<object, bool> isValidUrlParameters = null)
        {
            ...
            try
            {
                ...
                AppLogger
                   .Information<
                        HttpFunctionApiBase<TModel, TEntity, TId>>(messageTemplate: $"{logName} - Received Request",
                                                                   propertyValues: new Dictionary<string, object>
                                                                       { { "Data", json } });

                if (!id.Equals(request.Id)) throw new ArgumentException("Invalid Id provided.");
                ...
                AppLogger.Information<HttpFunctionApiBase<TModel, TEntity, TId>>(messageTemplate:
                    $"{logName} - Completed");
            }
            catch (ValidationRollupException validationRollupException)
            {
                AppLogger.Error<HttpFunctionApiBase<TModel, TEntity, TId>>(messageTemplate:
                                                                           $"{nameof(TModel)} - Validation Exception - {validationRollupException.Message}");
        ...
        protected async Task<HttpResponseData> Delete(HttpRequestData req,
                                                      TId id,
                                                      string logName,
                                                      Func<object, bool> isValidUrlParameters = null)
        {
            ...
            try
            {
                ...
                {
                    var result =
                        await Manager.Delete(entity); //This always returns true and will break if you pass a bad Id
                }
                catch
                {
        ...
        protected async Task<HttpResponseData> Patch(HttpRequestData req,
                                                     TId id,
                                                     string logName,
                                                     Func<object, bool> isValidUrlParameters = null)
        {
        ...
        protected async Task<HttpResponseData> HandleRequest(HttpRequestData req,
                                                             string logName,
                                                             Func<HttpResponseData, Task> processRequest)
        {
