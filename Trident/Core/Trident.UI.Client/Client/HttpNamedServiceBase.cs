...
namespace Trident.UI.Client
{
        ...
        where TThis : IServiceProxy
    {
        ...
        protected virtual async Task<Response<T, TErrorCodes>> SendRequest<T, TErrorCodes>(string method, string route, object data = null)
            where T : class
            where TErrorCodes : struct, Enum
        {
            return await SendRequest<Response<T, TErrorCodes>, T, TErrorCodes>(this.HttpServiceName, method, route, data);
        }
