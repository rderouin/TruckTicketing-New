...
[TestClass]
public class EDIFieldDefinitionSalesLineValidationTaskTest : TestScope<EDIFieldDefinitionSalesLineValidationTask>
{
    [DataTestMethod]
    [TestCategory("Unit")]
    public async Task Task_SalesLineEDIFieldValueValidation_ShouldPass_EDIData_IsValid()
    {
        // arrange
        ...
        var searchResults = new List<SalesLineEntity>
        {
            new()
            {
                Id = Guid.NewGuid(),
        ...
        var results = new SearchResults<SalesLineEntity, SearchCriteria> { Results = searchResults };

        scope.SalesLineProviderMock.Setup(x => x.Search(It.IsAny<SearchCriteria>(), It.IsAny<bool>()))
    ...
    private class DefaultScope : TestScope<EDIFieldDefinitionSalesLineValidationTask>
    {
