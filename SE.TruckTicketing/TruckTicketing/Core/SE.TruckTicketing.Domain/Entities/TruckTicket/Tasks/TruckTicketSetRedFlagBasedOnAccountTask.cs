...
public class TruckTicketSetRedFlagBasedOnAccountTask : WorkflowTaskBase<BusinessContext<TruckTicketEntity>>
{
    private readonly IProvider<Guid, AccountEntity> _accountProvider;

    public TruckTicketSetRedFlagBasedOnAccountTask(IProvider<Guid, AccountEntity> accountProvider)
    {
    ...
    public override async Task<bool> Run(BusinessContext<TruckTicketEntity> context)
    {
        ...
        {
            targetEntity.CustomerWatchListStatus = accountEntity.WatchListStatus;
            targetEntity.IsRedFlagged = accountEntity.WatchListStatus == WatchListStatus.Red;
        }
