...
public class FacilitiesRepository : CosmosEFCoreSearchRepositoryBase<FacilityEntity>
{
    public FacilitiesRepository(ISearchResultsBuilder resultsBuilder,
    ...
    {
        if (source is not IQueryable<FacilityEntity> typedSource)
        {
            return source;
        }
        ...
        if (keywords.HasText())
        {
            typedSource = typedSource.Where(x => x.SiteId.ToLower().Contains(keywords.ToLower()) ||
