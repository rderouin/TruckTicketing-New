...
public class RejectDefaultDensityFactorByWeight : WorkflowTaskBase<BusinessContext<FacilityEntity>>
{
    ...
    {
        var validDefaultDensities = context.Target.WeightConversionParameters.Where(x => x.IsEnabled && (x.EndDate >= DateTimeOffset.Now.Date || x.EndDate == null)).ToList();
        ...
        foreach (var group in defaultDensities)
        {
            ...
            foreach (var facilityServiceList in facilityServices)
            {
                ...
                if (!intersection.Any())
                {
                    ...
                    isOverlap = !isOverlap ? overlappingDensityRecordsWithFacilityService.Any() : isOverlap;
                }
            ...
            if (isOverlap)
            {
                context.ContextBag.TryAdd(FacilityWorkflowContextBagKeys.OverlappingFacilityServiceForDefaultDensitiesByWeight, true);
            }
        }
    ...
    {
        return Task.FromResult(context.Target is { WeightConversionParameters: { } } && context.Target.WeightConversionParameters.Any());
    }
