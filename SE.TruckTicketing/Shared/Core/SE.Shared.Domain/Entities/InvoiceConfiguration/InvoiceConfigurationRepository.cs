...
public class InvoiceConfigurationRepository : CosmosEFCoreSearchRepositoryBase<InvoiceConfigurationEntity>
{
    public InvoiceConfigurationRepository(ISearchResultsBuilder resultsBuilder, ISearchQueryBuilder queryBuilder, IAbstractContextFactory abstractContextFactory, IQueryableHelper queryableHelper) :
    ...
    public IQueryable<T> ApplyKeywordSearchImpl<T>(IQueryable<T> source, string keywords)
    {
        ...
        if (source is not IQueryable<InvoiceConfigurationEntity> typedSource)
        {
            return source;
        }
        ...
        if (keywords.HasText())
        {
            ...
            typedSource = typedSource.Where(e => e.Name.ToLower().Contains(value) ||
                                                 e.Description.ToLower().Contains(value) ||
                                                 e.CustomerName.ToLower().Contains(value));
        }
